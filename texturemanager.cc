...
bool TextureManager::load(string filePath, string id, SDL_Renderer *renderer) {
  ...
  SDL_Surface *tempSurface =
      IMG_Load(filePath.c_str()); // .c_str() because IMG_Load expects a char *
...
void TextureManager::draw(string id, int x, int y, int width, int height,
  ...
  SDL_RenderCopyEx(renderer, textureMap[id], &sourceRectangle,
                   &destinationRectangle, 0, 0,
                   flip); // the 0, 0 is for rotation
                          // (we _don't_ want to rotate so we ask the function
                          // to rotate 0 degrees about the centre 0)
}
...
void TextureManager::clearFromTextureMap(string id) { textureMap.erase(id); }
